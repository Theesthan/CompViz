...
class Codec {
	...
	while (this.heap.size() > 1) {
		let a = this.heap.top(); this.heap.pop();
		let b = this.heap.top(); this.heap.pop();
		this.heap.push([a[0] + b[0], [a, b]]);
	}

	let tree = this.heap.top(); this.heap.pop();
	this.codes = {};
	this.getCodes(tree, "");

	let bitstring = "";
	for (let ch of data) {
		bitstring += this.codes[ch];
	}
